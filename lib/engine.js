...
const { URL_SERVER, URL_BOT_SERVER, TIME_UPDATE_ORDER_BOOK } = require('./../url-config.json')
let clock
...
module.exports = function (s, conf) {
  ...
  function startServerConnection(socket) {
    socket.on('message', message => {
      ...
      if (message.type === 'notification' && message.payload.method === 'sendOrder') {
        ...
        const opts = parseOrderOptions(newOrder)
        if (opts.price) {
      ...
      if (message.type === 'notification' && message.payload.method === 'checkOrder') {
        ...
        checkOrderById(checkingOrder.arbitOrderId)
      }
    })
  }
  function checkOrderById(checkingOrder) {
    syncBalance((err, resBalance) => {
  ...
  function parseOrderOptions(order) {
    ...
    if (order.typeOrder === 'sell') {
      options.price = lastQuote.bids[0][0] > order.price * (1 - order.deviationPrice) ?
        order.price : undefined
      options.price = order.price
    } else {
      options.price = lastQuote.asks[0][0] < order.price * (1 + order.deviationPrice) ?
        order.price : undefined
    }
    ...
    s.exchange.getOrder(
      ...
      function (err, api_order) {
        ...
        if (api_order.status === 'done') {
          ...
          client.notification('trades', [
            [
              {
                exchange: exchangeName[0], pair: order.product_id, price: order.price, volume: order.orig_size,
                typeOrder: api_order.tradetype, idOrder: order.idOrderAb, exchOrderId: order.order_id, time: order.time
              }
            ],
            URL_BOT_SERVER,
            serverPort
