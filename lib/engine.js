...
function sendUDPMessage(targetIP, targetPORT, message) {
  //addressUdp = clientUdp.address();
  
  clientUdp.on("error", function(e) {
    throw e;
  });
  ...
  clientUdp.on("message", function(msg, rinfo) {
    ...
    console.log('msg :', msg);
    //console.log("UDP callback timestamp:", diff);
    //clientUdp.close();
  });
  //client.send(message, targetPORT, targetIP, function(err, bytes) {});

  clientUdp.on("listening", function() {
    addressUdp = clientUdp.address();
    console.log(
      "UDP Server listening on " + addressUdp.address + ":" + addressUdp.port
    );
  });
  //clientUdp.send(message, 0, message.length, targetPORT, targetIP, function(
  clientUdp.send(message, 0, message.length, targetPORT, targetIP, function(
    err,
    bytes
  ) {
...
module.exports = function(s, conf) {
  let closePrice, currentCurrency, currentVolume, nameSocket;
  ...
  function getOrderBook(cb) {
    if(s.trades[s.trades.length - 1]) {
      //console.log('====================s.trades[s.trades.length - 1].selector =  :', s.trades[s.trades.length - 1].selector);
   
    
     let nameForSocket = s.trades[s.trades.length - 1].selector;
    nameForSocket = nameForSocket.replace(".", " ");
    console.log('addressUdp :', addressUdp); 
    s.exchange.getDepth({ product_id: s.product_id, limit: 1 }, function(err, quote) {
     ...
     console.log('++++++++++++++++++++');
      const exchangeOrderBook = {
        exchange: nameForSocket,
        orderBook: quote,
       /*  host: addressUdp.address,
        port: addressUdp.port  */
      }
     
      let bufferOrderBook = Buffer.from(JSON.stringify(exchangeOrderBook));
      sendUDPMessage("0.0.0.0", 9999, bufferOrderBook);
  ...
  function executeOrder(trade, type) {
    ...
    if (s.buy_order) {
      ...
      let my_trade = {
        ...
        profit:
          s.last_sell_price && (s.last_sell_price - price) / s.last_sell_price,
        cancel_after: so.cancel_after || "day"
      };
      ...
      let my_trade = {
        ...
        profit:
          s.last_buy_price && (price - s.last_buy_price) / s.last_buy_price
      };
      s.my_trades.push(my_trade);
      if (so.stats) {
        let order_complete =
  ...
  function writeReport(is_progress, blink_off) {
    ...
    let nameForSocket = s.trades[s.trades.length - 1].selector;
    nameForSocket = nameForSocket.replace(".", " ");
    let packageForServer = {
      nameSocket: nameForSocket,
      closePrice: s.period.close,
      currency: s.currency,
      volume: s.period.volume,
      differentTime: 0,
      time: Date.now(),
      numberTrade: numberTrade,
      mashine: process.env.MASHINE
    }; 
    
    //let buf = Buffer.from(JSON.stringify(packageForServer));
    //console.log('s :', s);
    //sendUDPMessage("0.0.0.0", 9999, buf);
   // sendUDPMessage(URL_BOT_SERVER, 9999, Buffer(JSON.stringify(packageForServer)));
    client.request("Port", {
